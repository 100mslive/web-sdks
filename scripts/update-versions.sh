lerna version $1 --no-git-tag-version --exact --yes --no-private
lerna add @100mslive/react-ui --scope=100ms_edtech_template --exact
lerna add @100mslive/react-sdk --scope=100ms_edtech_template --exact
lerna add @100mslive/react-icons --scope=100ms_edtech_template --exact
lerna add @100mslive/hms-virtual-background --scope=100ms_edtech_template --exact
lerna add @100mslive/hms-noise-suppression --scope=100ms_edtech_template --exact
lerna add @100mslive/hls-stats --scope=100ms_edtech_template --exact
lerna add @100mslive/hls-controllers --scope=100ms_edtech_template --exact
lerna add @100mslive/react-icons --scope=100ms-custom-app --exact
lerna add @100mslive/react-ui --scope=100ms-custom-app --exact