...
export const NoiseSuppression = () => {
  ...
  const isPluginPresent = useHMSStore(selectIsLocalAudioPluginPresent('@100mslive/hms-noise-suppression'));
  ...
  const createPlugin = useCallback(async () => {
    if (!pluginRef.current) {
      const { HMSNoiseSuppressionPlugin } = await import('@100mslive/hms-noise-suppression');
  ...
  const addPlugin = useCallback(async () => {
    try {
      ...
      const pluginSupport = hmsActions.validateAudioPluginSupport(pluginRef.current);
