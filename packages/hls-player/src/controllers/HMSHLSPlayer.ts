...
export class HMSHLSPlayer implements IHMSHLSPlayer, IHMSHLSPlayerEventEmitter {
  ...
  constructor(hlsUrl: string, videoEl?: HTMLVideoElement) {
    ...
    this.listenHLSEvent();
    this._metaData = new HMSHLSTimedMetadata(this._hls, this._videoEl, this.emits);
  ...
  private subscribeStats = (interval = 2000) => {
    this._subscribeHlsStats = this._hlsStats.subscribe((state: HlsPlayerStats) => {
      this.emits(HMSHLSPlayerEvents.STATS, state);
    }, interval);
  };
  /**
  ...
  emits = <E extends HMSHLSPlayerEvents>(
    eventName: E,
    eventObject: Parameters<HMSHLSPlayerListeners<E>>[0],
  ): boolean => {
    return this._emitter.emits(eventName, eventObject);
  };
  ...
  private playVideo = async () => {
    try {
      ...
      if ((error as Error).name === 'NotAllowedError') {
        this.emits(HMSHLSPlayerEvents.AUTOPLAY_BLOCKED, HMSHLSErrorFactory.HLSMediaError.autoplayFailed());
      }
    }
  };
  private pauseVideo = () => {
  ...
  private playEventHandler = () => {
    this.emits(HMSHLSPlayerEvents.PLAYBACK_STATE, {
  ...
  private manifestLoadedHandler = (_: any, { levels }: { levels: LevelParsed[] }) => {
    ...
    this.emits(HMSHLSPlayerEvents.MANIFEST_LOADED, {
      layers,
    });
  };
  private levelUpdatedHandler = (_: any, { level }: { level: number }) => {
    const qualityLayer: HMSHLSLayer = mapLayer(this._hls.levels[level]);
    this.emits(HMSHLSPlayerEvents.LAYER_UPDATED, {
      layer: qualityLayer,
    });
  };

  private handleTimeUpdateListener = (_: Event) => {
    this.emits(HMSHLSPlayerEvents.CURRENT_TIME, this._videoEl.currentTime);
    ...
    if (this._isLive !== live) {
      this._isLive = live;
      this.emits(HMSHLSPlayerEvents.SEEK_POS_BEHIND_LIVE_EDGE, {
        isLive: this._isLive,
      });
    }
  };
