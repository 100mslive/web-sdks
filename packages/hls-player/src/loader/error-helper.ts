import { ErrorData, ErrorDetails, LoaderConfig, LoaderResponse, LoadPolicy, RetryConfig } from 'hls.js';
...
export function getRetryConfig(loadPolicy: LoadPolicy, error: ErrorData): RetryConfig | null {
  const isTimeout = isTimeoutError(error);
  return loadPolicy.default[`${isTimeout ? 'timeout' : 'error'}Retry`];
}

export function getRetryDelay(retryConfig: RetryConfig, retryCount: number): number {
  // exponential backoff capped to max retry delay
  const backoffFactor = retryConfig.backoff === 'linear' ? 1 : Math.pow(2, retryCount);
  return Math.min(backoffFactor * retryConfig.retryDelayMs, retryConfig.maxRetryDelayMs);
}

export function getLoaderConfigWithoutReties(loderConfig: LoaderConfig): LoaderConfig {
  return {
    ...loderConfig,
    ...{
      errorRetry: null,
