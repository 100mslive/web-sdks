...
import { HLSController } from '../interfaces/HLSController';
...
export class HLSJSController implements HLSController {
  hls?: Hls;
  videoEle;
  eventEmitter? = new EventEmitter();
  isLive = true;
  ControllerEvents;
  constructor(hlsUrl: string, videoEle: HTMLMediaElement, isOptimized: boolean) {
    this.hls = new Hls(this.getHLSConfig(isOptimized));
    this.videoEle = videoEle;
    this.hls.loadSource(hlsUrl);
    this.hls.attachMedia(videoEle);
    this.handleHLSTimedMetadataParsing();
    this.ControllerEvents = [HLS_TIMED_METADATA_LOADED, HLS_STREAM_NO_LONGER_LIVE];
  }
   ...
   */
  setCurrentLevel(currentLevel: Hls.Level) {
  ...
  enableTimeUpdateListener() {
    this.videoRef.current.addEventListener('timeupdate', _ => {
      const videoEl = this.videoRef.current;
      if (this.hls && videoEl) {
        const allowedDelay = this.getHLSConfig().liveMaxLatencyDuration || HLS_DEFAULT_ALLOWED_MAX_LATENCY_DELAY;
        this.isLive = this.hls.liveSyncPosition - videoEl.currentTime <= allowedDelay;
        if (!this.isLive) {
  ...
  handleHLSTimedMetadataParsing() {
    ...
    this.hls.on(Hls.Events.FRAG_CHANGED, (_, { frag }) => {
      try {
        if (this.videoEle.textTracks.length === 0) {
          return;
        }
         ...
         */
        const metadata = [...this.videoEle.textTracks[0].cues];
        /**
  ...
  getHLSConfig(isOptimized: boolean) {
    if (isOptimized) {
